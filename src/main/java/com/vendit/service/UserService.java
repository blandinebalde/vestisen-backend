...
public class UserService {
    ...
    /**
     * Recalcule et persiste le sceau pour les utilisateurs dont la colonne est vide (post-migration).
     * Contrôlé par {@code app.security.auto-repair-privilege-seals} au démarrage.
     */
    public int repairUsersWithMissingPrivilegeSeals() {
        List<User> missing = userRepository.findUsersWithMissingPrivilegeSeal();
        int n = 0;
        for (User u : missing) {
        ...
        if (n > 0) {
            logger.info("Sceaux de privilège initialisés pour {} utilisateur(s).", n);
        }
        return n;
    }

    /**
     * Met à jour le profil de l'utilisateur (nom, prénom, téléphone, adresse, whatsapp).
     */
    public User updateProfile(Long userId, ProfileUpdateRequest request) {
        ...
        if (request.getAddress() != null) user.setAddress(request.getAddress().trim().isEmpty() ? null : request.getAddress().trim());
        if (request.getWhatsapp() != null) user.setWhatsapp(request.getWhatsapp().trim().isEmpty() ? null : request.getWhatsapp().trim());
        return userRepository.save(user);
    }
     ...
     */
    public User setAvatarPath(Long userId, String avatarPath) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));
    ...
    public void initializeAdmin() {
        if (!userRepository.existsByEmail(adminEmail)) {
            ...
            if (admin != null) {
                ...
                if (admin.getPrivilegeSeal() == null || admin.getPrivilegeSeal().isBlank()) {
                    needsUpdate = true;
                }
    ...
    /**
     * Comptes de démo (vendeur + client) si absents. À appeler après {@link #initializeAdmin()}.
     * Idempotent.
     */
    public void ensureSeedDemoUsers() {
        createSeedUserIfAbsent(
                "vendeur@vendit.com",
                "vendeur123",
                "Aminata",
                "Diallo",
    ...
    private void createSeedUserIfAbsent(String email, String plainPassword, String firstName, String lastName,
        ...
        u.setCode(generateUniqueUserCode());
        userRepository.save(u);
        logger.info("Seed user created: {}", email);
    }

    /**
     * Active ou désactive un compte (admin). Désactivation : invalide les JWT en cours via tokenVersion.
     */
    public User setEnabledByAdmin(UUID publicId, boolean enabled) {

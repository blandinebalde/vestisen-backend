...
/**
 * Annonce marketplace.
 * <p>Table physique MySQL : {@code annonces} (pluriel) — ne pas chercher {@code annonce}.
 * Hibernate crée ou met à jour cette table au démarrage si {@code spring.jpa.hibernate.ddl-auto}
 * est {@code update} (ou {@code create}) et que le contexte Spring termine sans erreur de schéma
 * (une FK incohérente peut empêcher toute création de tables).
 */
@Entity
@Table(name = "annonces")
@Data
...
public class Annonce {
    ...
    @Column(name = "article_condition", length = 50)
     ...
     * <p>Contrainte explicite {@code fk_annonce_buyer} sur {@code buyer_id} → {@code users} pour éviter
     * les noms auto-générés par Hibernate (ex. {@code FKgmmop32kneepq9dnh4ksd7eh6}) qui peuvent provoquer
     * des erreurs de métadonnées au démarrage. Si une ancienne FK bloque encore le démarrage, voir
     * {@code backend/scripts/mysql-drop-problematic-fk.sql}.
     */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "buyer_id", foreignKey = @ForeignKey(name = "fk_annonce_buyer"))
    @ToString.Exclude
    @EqualsAndHashCode.Exclude
    private User buyer;
